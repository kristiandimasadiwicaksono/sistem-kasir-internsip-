...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        @if(session('verified'))
            <div class="mt-6 text-center bg-green-50 rounded-xl p-6 border border-green-200 shadow-sm">
                ...
                <h3 class="text-lg font-semibold text-green-700">Email berhasil diverifikasi!</h3>
                <p class="mt-1 text-sm text-green-600">Anda akan diarahkan ke Dashboard secara otomatis.</p>

                <!-- Progress bar -->
                <div class="w-full bg-green-100 h-2 rounded-full mt-4 overflow-hidden">
                    <div id="progress-bar" class="bg-green-500 h-2 rounded-full transition-all duration-1000"></div>
                </div>

                <!-- Countdown text -->
                <p id="redirect-message" class="mt-3 text-sm text-slate-500 font-medium"></p>
            </div>

            <script>
                let seconds = 5; // ⏳ waktu tunggu
                let message = document.getElementById("redirect-message");
                let progress = document.getElementById("progress-bar");

                function updateCountdown() {
                    message.innerText = `Redirect dalam ${seconds} detik...`;
